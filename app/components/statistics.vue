  ...
  <div class="font-['Montserrat'] relative w-full">
    <div class="flex flex-wrap items-center gap-8 text-xl font-bold text-muted">
      ...
      <button 
        ...
        class="relative z-[1] flex cursor-pointer items-center gap-2 border-none bg-transparent p-0 transition-opacity hover:opacity-70 focus:outline-none"
      >
        <span>Підписок</span>
      ...
      <Teleport to="body">
        ...
        >
          ...
          >
            ...
            >
              <div class="relative w-full max-w-sm overflow-hidden rounded-2xl border-4 border-plum-light bg-mauve shadow-2xl">
                ...
                <div class="custom-scrollbar max-h-[70vh] overflow-y-auto p-6">
                  ...
                  <div class="space-y-3">
                    <div 
                      ...
                      :key="item.id"
                      class="flex items-center gap-4 rounded-xl border border-plum-light/20 bg-white/30 p-3 transition-colors hover:bg-white/50"
                    >
                      <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-plum-light text-white">
                        <span v-if="modalType === 'works'" class="text-lg">📖</span>
                        <span v-else class="font-bold">{{ item.name.charAt(0).toUpperCase() }}</span>
                      </div>
                      
                      <span class="text-[1.1rem] font-medium text-muted line-clamp-2 leading-tight">
                        {{ item.name }}
                      </span>
                    </div>

                    <div v-if="currentList.length === 0" class="py-6 text-center font-medium text-plum-light/60">
                      Список порожній.
                    </div>
                  </div>
