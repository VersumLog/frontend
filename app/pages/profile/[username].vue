...
<template>
  <div class="profile-container">
    ...
    <!-- searching animation -->
    <div v-if="!userData && !error" class="flex flex-col items-center justify-center min-h-[60vh] space-y-6 bg-cream rounded-[40px]">
      <div class="relative flex items-center justify-center">
        <!-- glow -->
        <div class="absolute animate-ping h-20 w-20 rounded-full bg-plum opacity-10"></div>

        <!-- circle -->
        <div class="relative rounded-full h-14 w-14 border-4 border-plum-light border-t-[var(--color-plum)] animate-spin"></div>

        <!-- icon -->
        <span class="absolute text-xl animate-spin">
          <Icon name="tabler:user-search" class="w-12 h-12 text-plum" />
        </span>
      </div>
      ...
      <div class="text-center space-y-3">
        ...
        <div class="inline-flex items-center px-4 py-1.5 rounded-full bg-plum-light/5 border border-plum-light/10">
          <span class="text-plum font-semibold text-sm">
            ...
            <span class="underline decoration-wavy decoration-plum-light/30">@{{ username }}</span>
          </span>
        </div>
      </div>
    ...
    <div class="bg-white/50 p-12 rounded-[40px] border-2 border-dashed border-plum-light/20 max-w-lg w-full">
      ...
      <h2 class="text-3xl font-black text-plum-light mb-2">Користувача не знайдено</h2>
      <p class="text-plum-light/70 mb-8">
        На жаль, профілю з нікнеймом <span class="font-bold text-plum-light">@{{ username }}</span> не існує або він був видалений.
      </p>

      <div class="flex flex-col sm:flex-row gap-4 justify-center">
        <button @click="navigateTo('/')" 
          class="bg-plum-light text-white px-8 py-3 rounded-xl font-bold hover:bg-plum-scroll-hover transition-colors">
          На головну
        </button>
      </div>
    </div>
